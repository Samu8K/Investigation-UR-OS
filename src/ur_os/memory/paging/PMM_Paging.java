...
import ur_os.virtualmemory.PVMM_MRU;
...
public class PMM_Paging extends ProcessMemoryManager{
    ...
    public int getFrameMemoryAddressFromLogicalMemoryAddress(int page){
        MemoryAddress addr = getFrameMemoryAddressFromLogicalMemoryAddress(new MemoryAddress(page, 0));
        return (addr != null) ? addr.getDivision() : -1;  // ✅ Seguridad ante null
}
...
public int getVictim() {
    if (this.loadedPages != this.assignedPages) return -1;
    
    return pvmm.getVictim(memoryAccesses, this.loadedPages, this.getPT());
}



    public boolean isPageValid(int page) {
    return vpt.isPageValid(page);
}
